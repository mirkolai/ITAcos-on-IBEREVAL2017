...
for L in range(1, 1+1):
    for subset in combinations(stuff, L):
        ...
        for clfname,clf in clfs.items():
            ...
            clf["clf"].fit(X_train,stance_train)
...
for i in range(0,len(tweets_test)):
    ...
    majority_voting.append(stance)


for i in range( 0,len(tweets_test)):
    print(majority_voting[i])
    print(tweets_test[i].id)
    cur.execute(" INSERT INTO `test_5`"
                " (`id`, `language`, `stance` ) "
                " VALUES (%s,%s,%s) "
                " on  duplicate key  update `stance`=%s ",(tweets_test[i].id,tweets_test[i].language,majority_voting[i],majority_voting[i],))
